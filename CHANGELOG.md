...
- Inspector 去掉复制、归档、取消等和笔记卡片三点菜单重复的低频动作，顶部只保留跳转、编辑、完成 / 重新打开、迁移和移到 Future 等上下文操作。
- Inspector 顶部工具栏改回单行图标按钮，避免按钮换成两行导致面板显得拥挤。
- Inspector 标题改用清洗后的笔记摘要，不再直接显示裸 Markdown。
- Inspector 内容区域改为更完整的卡片样式，保留条目类型侧边色条，和主界面的卡片语言保持一致。
- Related Notes 复用压缩后的笔记卡片展示，并使用后端摘要，减少重复文字和无效来源信息。

## v0.5.6 - 2026-06-20

- Inspector 从只读预览升级为可操作面板，可直接跳转到对应日期或 Future Log。
- Inspector 支持在面板内编辑正文、类型、普通状态和标签，并支持直接勾选 Markdown checklist。
- Related Notes 支持在 Inspector 内前进 / 后退浏览，点击相关笔记不会丢失浏览历史。
- Related Notes 卡片去掉“语义相关”等来源胶囊，改为更柔和的卡片样式。
- Inspector 顶部集成完成、迁移、移到 Future、归档、取消和复制等常用动作；日期迁移继续走原有迁移流程，避免破坏迁移链。

## v0.5.5 - 2026-06-20

- 搜索弹窗移除手动模式切换，普通输入默认走智能语义搜索，输入 `/关键词/` 时自动使用正则搜索。
- Inspector 支持点击面板外空白和按 ESC 关闭，条目三点菜单也支持点击外部和 ESC 收起。
- 本地安全快照新增恢复能力；恢复前会自动创建一份“恢复前”快照，方便再回到恢复前状态。
- 快照恢复会重写本地笔记数据库和 Daily/Future Markdown，附件文件保持不删除，避免误删真实文件。

## v0.5.4 - 2026-06-20

- 新增右侧 Inspector 面板，单击笔记可查看完整内容、时间、标签和 Related Notes，双击正文仍然进入编辑。
- “数据与备份”面板新增本地安全快照，快照存放在 Markdown 项目文件夹 `.bujo/snapshots/`，默认保留最近 7 份，并只记录附件清单不重复打包附件内容。
- 自动本地快照会在桌面端启动时静默尝试创建，可在面板中手动立即保存，也可以暂停 7 天后再恢复。
- 搜索默认走智能语义模式，语义无结果时自动回退全文匹配；空结果时继续提供全文和正则入口，但不增加搜索框复杂度。
- 升级 Rust / Tauri 相关依赖，保持桌面后端依赖在当前稳定版本。
