...
- 主页月历主卡片放大，默认桌面布局下日期格、标题和卡片宽度更舒展。
- Daily Sheet 顶部手柄改为可拖拽调整日历和当天内容的高度比例，日历被压低时会自动用周视图渲染，不覆盖用户保存的月视图偏好。
- 日历状态圆点改到日期数字下方的独立行，避免选中日期圆和圆点 / 数量徽标互相挤压。
- Markdown 中的普通链接和 `file://` 本地文件链接现在会交给系统浏览器或默认应用打开，不再让 Tauri WebView 跳离主页面。

## v0.6.0 - 2026-06-21

- 新增右侧常驻 Workbench：时间线、笔记详情、编辑、迁移、移到 Future 和 Related Notes 集中在一个可收起、可调整宽度的面板里。
- 主页移除重复的 Timeline 顶部按钮和命令面板入口，并删除旧 Timeline 页面 / 弹窗实现，统一使用右侧 Workbench。
- 日历在窗口高度不足时会临时自动收紧为周视图，不改写用户保存的月视图偏好；窗口变高后会回到月视图。
- 优化日历紧凑布局下的日期圆点、状态图标和数量徽标，避免低高度窗口里标记挤到日期数字上。
- Future Log 调度补齐“待定”和常用月份目标，迁移到 Future 时保留原日期侧的迁移印记。
- Workbench 详情页复用 Markdown 工具栏和胶囊式状态控件，减少额外弹窗并保持和主界面一致的卡片风格。
