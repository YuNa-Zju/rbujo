...
test("desktop attachment commands are exposed for portable uploads", async () => {
  ...
  assert.match(markdownViewerSource, /parsed\.protocol === "file:"/);
  assert.match(markdownViewerSource, /parsed\.hostname === "asset\.localhost"/);
  assert.match(markdownViewerSource, /handleMarkdownLinkOpen/);
  assert.match(markdownViewerSource, /event\.preventDefault\(\)/);
  assert.match(markdownViewerSource, /event\.stopPropagation\(\)/);
  assert.match(markdownViewerSource, /entryService\.openExternalLink/);
  ...
  assert.match(source, /open_external_link/);
  assert.match(source, /external_open_target/);
  assert.match(entryServiceSource, /openExternalLink/);
  assert.match(entryServiceSource, /invoke<void>\("open_external_link"/);
